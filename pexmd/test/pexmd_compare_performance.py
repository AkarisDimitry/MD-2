...
data = np.zeros( (140, 4) ) 
Fdata = np.zeros( (140, 4) )
Edata = np.zeros( (140,4,3) )
...
for n in range(10,150):
    ...
    while var00 < steps:
        ...
        #print time.clock()-t1
        data[n-10, 0] += time.clock()-t1
        Edata[n-10,0,0] = E
        Edata[n-10,0,1] = np.sum(sistema00.v)**2 / 2
        Edata[n-10,0,2] = E + np.sum(sistema00.v)**2 / 2
...
color = ( 'r', 'g', 'b', 'k')
plt.figure(1)
plt.xlabel( 'Numero de particulas', fontsize=32 )
plt.ylabel( 'Tiempo promedio de 5 steps ' , fontsize=32)
...
plt.figure(2)
for i, n in enumerate(color): plt.plot(Edata[:,i,:], n)
